...
def write_review(predictions: List[Dict[str, Any]], path: Path | str) -> None:
    ...
    for i, row in enumerate(predictions, 1):
        ...
        if row.get("route_type"):
            lines.append(f"**Route type:** {row['route_type']}")
            lines.append("")
        if row.get("vector_query") and row.get("vector_query") != row.get("search_query"):
            lines.append(f"**Vector query:** {row['vector_query']}")
            lines.append("")
        if row.get("bm25_query") and row.get("bm25_query") != row.get("search_query"):
            lines.append(f"**BM25 query:** {row['bm25_query']}")
            lines.append("")
        if row.get("metadata_filters"):
            lines.append(f"**Metadata filters:** {row['metadata_filters']}")
            lines.append("")
        if row.get("retrieved_chunk_ids"):
