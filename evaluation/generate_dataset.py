...
def _resolve_chunks_dir(chunks_dir: Optional[Path], data_path: Optional[Path]) -> Path:
    if chunks_dir is not None:
        return Path(chunks_dir).resolve()
    if data_path is not None:
        return Path(data_path).resolve() / "chunks"
    return _default_chunks_dir()
...
def main() -> None:
    ...
    p_llm.add_argument(
        ...
        type=Path,
        default=None,
        help="数据根目录；未指定 --chunks-dir 时使用 <data-path>/chunks",
    )
    ...
    chunks_dir = _resolve_chunks_dir(
        getattr(args, "chunks_dir", None),
