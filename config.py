...
class RAGConfig:
    ...
    def resolve_index_path(self) -> Path:
        p = Path(self.index_save_path)
        if p.is_absolute():
            return p
        return (_PROJECT_ROOT / p).resolve()

    def with_storage_paths(
        self,
        *,
        ...
        if data_path:
            updates["data_path"] = data_path.strip()
        if index_path:
            updates["index_save_path"] = index_path.strip()
...
def prompt_storage_paths(config: RAGConfig) -> RAGConfig:
    """交互式选择数据目录与向量索引目录。"""
    print("\n--- 数据与索引路径 ---")
    print(f"  数据目录（NASA chunks/PDF）: {config.resolve_data_path()}")
    print(f"  向量索引目录（FAISS）:       {config.resolve_index_path()}")
    data_in = input("数据目录 [回车保持当前]: ").strip()
    index_in = input("向量索引目录 [回车保持当前]: ").strip()
    cfg = config.with_storage_paths(data_path=data_in or None, index_path=index_in or None)
    print(f"  -> 数据: {cfg.resolve_data_path()}")
    print(f"  -> 索引: {cfg.resolve_index_path()}")
    return cfg
