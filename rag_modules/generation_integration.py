...
class GenerationIntegrationModule:
    ...
    def setup_llm(self):
        if self.llm_provider == "local":
            ...
            logger.info(f"正在初始化本地 LLM: {model} @ {base_url}")
        else:
            ...
            logger.info(f"正在初始化 OpenAI LLM: {model} @ {base_url}")
...
English:""",
        ...
        try:
            ...
            if result:
                logger.info(f"检索词 LLM 翻译: '{query}' -> '{result}'")
                return result
        ...
        if deepl_result:
            logger.info(f"检索词 DeepL 翻译: '{query}' -> '{deepl_result}'")
            return deepl_result

        logger.warning("翻译失败，检索将使用原始中文查询")
        return query
...
Classification:""")
    ...
    def generate_basic_answer_stream(self, query: str, context_docs: List[Document]):
        ...
        prompt = ChatPromptTemplate.from_template("""
""" + AEROSPACE_SYSTEM_HINT + """
Answer based on context. Use the same language as the question.
...
Answer:""")
    ...
    def generate_step_by_step_answer_stream(self, query: str, context_docs: List[Document]):
        context = self._build_context(context_docs)
        prompt = ChatPromptTemplate.from_template("""
Provide a structured detailed aerospace engineering answer. Use the same language as the question.
