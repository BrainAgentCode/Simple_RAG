...
from .runtime_accel import gpu_retrieval_lock
...
class RetrievalOptimizationModule:
    ...
    def hybrid_search(
        ...
        Args:
            query: 默认查询（未指定 vector/bm25 时共用）
            top_k: 返回结果数量
            vector_query: 向量检索用词（可与 BM25 不同，如中文问句 + 英文 BM25）
            bm25_query: BM25 检索用词
        ...
        if self.serialize_gpu_retrieval:
            with gpu_retrieval_lock():
                vector_docs = self.vector_retriever.invoke(vq)
        else:
            vector_docs = self.vector_retriever.invoke(vq)
