...
├── POST /api/chat      → 发送消息
├── POST /api/clear_all → 清空所有对话
├── POST /api/switch_conv → 切换对话
├── POST /api/delete_conv → 删除对话
├── POST /api/settings  → 保存设置
├── GET  /api/kb/files  → 获取知识库文件列表
└── POST /api/kb/rebuild → 重建知识库索引

Frontend (static/index.html)
└── UltraRAG 风格纯 HTML/CSS/JS，通过 fetch API 与后端通信
```
...
│   ├── start_fastapi.py       # 后台启动脚本
│   └── static/
│       └── index.html         # UltraRAG 风格前端
├── .env.example               # 环境变量模板
├── requirements.txt
├── rag_modules/
...
所有设置通过 WebUI 系统设置面板配置：

| 设置分组 | 包含内容 |
|----------|----------|
| 大语言模型 (LLM) | 提供商、API Key、Base URL、模型名称、Temperature、Max Tokens、最大并发数 |
| 向量嵌入 | 嵌入模型、设备(auto/cpu/cuda)、FAISS GPU、批次大小 |
| 检索设置 | Top K、查询翻译用于 BM25/向量检索 |
| Reranker 精排 | 启用开关、模型、设备 |
| 翻译 / DeepL | DeepL API Key、URL、翻译模式 |

知识库设置通过知识库页面的「知识库设置」按钮配置：
- 数据目录（只读显示）
- 向量索引目录（只读显示）
- 文件数量和索引状态
